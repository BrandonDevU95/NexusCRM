...
- Use TypeORM with PostgreSQL, explicit entities and versioned migrations from the first phase.
- Use manual modular seeds, never automatic app-start seeding.
- Use OpenSpec-style SDD files tracked in Git.

## Backend boundaries

Each backend module owns its controller/service/repository/policies/events/seeds/tests folders. Controllers must not access TypeORM directly.
...
- TypeORM entities and migrations may evolve heavily during early auth/CRM phases. Mitigation: keep the foundation model minimal and keep schema synchronization disabled.
